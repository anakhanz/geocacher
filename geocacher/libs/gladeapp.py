...
class GladeApp:
    ...
    def __init__(self,*args,**dargs):
        ...
        assert hasattr(self,"glade"), "* ERROR* missing attribute 'glade' in '%s'"%n
        assert hasattr(self,"init"), "*ERROR* missing method 'init' in '%s'"%n
        ...
        assert len(l)>=1,"*ERROR* no widgets in the .glade"

        # controle le matching object avec une ou la window du glade
        if len(l)>1:
            theId,theNode=None,None
            assert hasattr(self,"window"), "*ERROR* missing attribute 'window' in '% s' (as in the glade file has more than one) "%n
            for id,node in l:
                if self.window == id:
                    theId,theNode=id,node
                    break;
            assert theNode!=None, "*ERROR* the attribute 'window' in '% s' does not correspond to any window in the glade file "%n
        else:
            theId,theNode=l[0]
        ...
        for i in re.findall(r"""<signal handler="([^"]+)" """,theNode.toxml()):
            if not hasattr(self,i):
                print "*WARNING* missing method in '%s'"%n
