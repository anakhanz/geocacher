...
SELECT c.id AS id,
       ...
       (SELECT max(l.log_date) FROM Logs l where l.cache_id = c.id) AS last_log,
       (SELECT max(l.log_date) FROM Logs l where l.cache_id = c.id AND l.type = "Found it") AS last_found,
